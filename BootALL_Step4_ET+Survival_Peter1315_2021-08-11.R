...
load(file='DDbintau_boot_Peter1315.Rdata')
fname=c('ET+Sboot_Peter1315.Rdata')
# or
...
ETfun = for(iOK in 1:NOK)  {  # usually 1:NOK
  ...
  x = seq( (xeq[1]-0.1),xeq[2],length.out=10) # mesh of 20 (vs 30) does not blow up bvptwp
  ...
  x = seq(xeq[2],(xeq[3] + 0.1),length.out=10) # mesh of 20 (vs 30) does not blow up bvptwp
  # solve with bvpcol or bvptwp
  trycol <- tryCatch({
    bvptwp(yini = yini, x = x, func = feval2, yend = yend, parm=plist)
  },error=function(e){cat("ERROR :",conditionMessage(e), "\n")} )
  ...
  N = 1000
  xs = seq(from=xeq[1]-0.5,to=(xeq[2]-0.01),length.out=N) # left basin
  dx = diff(range(xs))/N
  ...
  # Make grid for ode.1D
  N = 1000
  xs = seq(from=(xeq[2]+0.01),to=(xeq[3]+1),length.out=N)
  dx = diff(range(xs))/N
  # Langevin components on mesh
  drif = D1fun(xs)
  dif = D2fun(xs)
